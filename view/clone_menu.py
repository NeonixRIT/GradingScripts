...
class CloneMenu(SubMenu):
    ...
    def __init__(self, id):
        ...
        manage_presets = MenuOption(
            1, 'Manage Presets', Event(), Event(), Event(), pause=False
        )
        ...
        clone_history = MenuOption(
            3, 'Clone History', Event(), Event(), Event(), pause=False
        )
        ...
        clone_repos = MenuOption(
            4, 'Continue Without Preset', clone_repos_event, Event(), Event()
        )
    ...
    def clone_repos(self, preset: ClonePreset = None):
        setattr(self.context, 'dry_run', bool(self.dry_run))
    ...
    def build_preset_options(self) -> list:
        ...
        for i, preset in enumerate(
            list_to_multi_clone_presets(self.context.config_manager.config.presets)
        ):
            option_event = Event()
...
class LocalRepo:
    ...
    async def get_commit_hash(self, date_due: str, time_due: str) -> str:
        ...
        try:
            ...
            print(
                f'{LIGHT_RED}[{self}] Get Commit Hash Failed: Likely accepted assignment after given date/time.{WHITE}'
            )
    ...
    async def rollback(self, commit_hash: str) -> bool:
        ...
        try:
            ...
            print(
                f'{LIGHT_RED}[{self}] Rollback Failed: Likely invalid filename at commit `{commit_hash}`.{WHITE}'
            )
