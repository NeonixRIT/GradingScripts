...
class CloneHistoryMenu(SubMenu):
    def __init__(self, id):
        clear_history_options = MenuOption(
            1, 'Clear History', Event(), Event(), Event(), pause=False
        )

        clear_history_options.on_select += (
            lambda: self.context.config_manager.set_config_value('clone_history', [])
        )
        ...
        self.legend = (
            f'{CYAN}{assign_str}{LIGHT_GREEN}{due_str}{LIGHT_RED}{curr_str}{WHITE}'
        )
    ...
    def load(self):
        ...
        self.prompt_string = (
            self.prompt_string
        ) = f'Please enter a number {LIGHT_GREEN}({self.min_options}-{self.max_options}){WHITE} or {LIGHT_RED}q/quit{WHITE} to return to the previous menu: '
    ...
    def build_preset_options(self) -> list:
        ...
        for i, report in enumerate(
            self.context.config_manager.config.clone_history[::-1]
        ):
            option_event = Event()

            report_str = (
                f'{CYAN}Assignment Name: {report.assignment_name}\n'
                ...
                else f'Tag Name: {getattr(report, "tag_name", None)}\n'
...
def report_summary(report):
    assignment_tag_str = (
        f'{report.assignment_name}:{report.dry_run}'.ljust(25)
        if getattr(report, 'dry_run', False)
        else f'{report.assignment_name}'.ljust(25)
    )
    due_str = f'{report.due_date} {report.due_time}'.ljust(20)
    curr_str = f'{report.current_date} {report.current_time}'.ljust(20)
    return (
        f'{CYAN}{assignment_tag_str}{LIGHT_GREEN}{due_str}{LIGHT_RED}{curr_str}{WHITE}'
    )
